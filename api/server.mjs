...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/admin/users") {
      await requireRole(req,["admin"]); const [rows,auth,assignments,definitions]=await Promise.all([supabase("/rest/v1/profiles?select=id,display_name,first_name,last_name,role,trust_score,report_count,suspended_until,created_at&order=created_at.desc&limit=200"),authAdmin("/users?page=1&per_page=1000"),supabase("/rest/v1/user_roles?select=user_id,role_key,expires_at"),supabase("/rest/v1/rbac_roles?active=eq.true&select=key,display_name")]);const now=new Date().toISOString(),authById=new Map((auth.users||[]).map(item=>[item.id,item])),roleNames=new Map(definitions.map(role=>[role.key,role.display_name]));return json(res,200,{users:rows.map(row=>{const roles=assignments.filter(item=>item.user_id===row.id&&(!item.expires_at||item.expires_at>now)).map(item=>({key:item.role_key,display_name:roleNames.get(item.role_key)||item.role_key.replaceAll("_"," "),privileged:item.role_key!=="user"}));if(!roles.length)roles.push({key:row.role==="admin"?"super_admin":row.role||"user",display_name:row.role==="admin"?"Super administrator":roleNames.get(row.role)||row.role||"User",privileged:row.role!=="user"});return{...row,email:authById.get(row.id)?.email||null,last_sign_in_at:authById.get(row.id)?.last_sign_in_at||null,mfa_factor_count:(authById.get(row.id)?.factors||[]).length,roles}})},origin);
    }
