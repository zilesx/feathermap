...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if(req.method==="GET"&&url.pathname==="/api/map/heatmap"){
      rateLimit(req,120);const mapConfig=await configValue("map_layers",{minimum_heat_reports:3,grid_degrees:4,heatmap_max_zoom:8,cluster_max_zoom:8});const range=requestedRange(url,180);const zoom=Math.max(2,Math.min(9,Number(url.searchParams.get("zoom"))||3));const adaptiveGrid=zoom<3?6:zoom<4?4:zoom<5?2:zoom<6?1:zoom<7?.5:zoom<8?.25:.12;const configuredGrid=Number(mapConfig.grid_degrees);const grid=Number.isFinite(configuredGrid)&&configuredGrid>0?Math.min(configuredGrid,adaptiveGrid):adaptiveGrid;const rows=await supabase("/rest/v1/rpc/activity_heatmap",{method:"POST",token:ANON_KEY,data:{p_since:range.start.toISOString(),p_grid_degrees:grid,p_minimum:Math.max(3,Number(mapConfig.minimum_heat_reports)||3)}});return json(res,200,{cells:rows,range,grid_degrees:grid,heatmap_max_zoom:Number(mapConfig.heatmap_max_zoom)||8,local_marker_zoom:Number(mapConfig.cluster_max_zoom)||8},origin);
    }
