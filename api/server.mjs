...
async function revokeUserSessions(userId){await authAdmin(`/users/${userId}/logout`,{method:"POST"});await supabase(`/rest/v1/app_sessions?user_id=eq.${userId}&revoked_at=is.null`,{method:"PATCH",data:{revoked_at:new Date().toISOString()},prefer:"return=minimal"})}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if(req.method==="POST"&&adminSuspend){
      ...
      await auditRequest(req,actor.user.id,"user.suspend","profile",targetId,{reason,duration,suspended_until:suspendedUntil});
      await securityEvent(req,"account_suspended","success",targetId,{actor_id:actor.user.id,duration});
      ...
      return json(res,200,{message:"Account suspended and active sessions revoked",suspended_until:suspendedUntil},origin);
    }
