...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if(req.method==="GET"&&url.pathname==="/api/feedback"){const{user}=await activeUser(req);const rows=await supabase(`/rest/v1/product_feedback?submitted_by=eq.${user.id}&select=id,category,title,status,priority,resolution_summary,created_at,updated_at&order=created_at.desc&limit=100`);return json(res,200,{feedback:rows},origin)}
    ...
    if(req.method==="GET"&&adminFeedback){await requirePermission(req,"feedback.view");const rows=await supabase(`/rest/v1/product_feedback?id=eq.${adminFeedback[1]}&select=*`);if(!rows.length)throw Object.assign(new Error("Feedback not found"),{status:404});const[messages,attachments,submitters]=await Promise.all([supabase(`/rest/v1/product_feedback_messages?feedback_id=eq.${adminFeedback[1]}&select=*&order=created_at.asc`),supabase(`/rest/v1/product_feedback_attachments?feedback_id=eq.${adminFeedback[1]}&select=id,mime_type,byte_size,created_at&order=created_at.asc`),supabase(`/rest/v1/profiles?id=eq.${rows[0].submitted_by}&select=id,first_name,last_name,display_name`)]);return json(res,200,{feedback:{...rows[0],submitter:submitters[0]||null},messages,attachments:attachments.map(item=>({...item,url:`/api/feedback/attachments/${item.id}`}))},origin)}
    if(req.method==="PATCH"&&adminFeedback){const{user}=await requirePermission(req,"feedback.manage"),input=await body(req),update={updated_at:new Date().toISOString()};if(["new","reviewing","planned","in_progress","resolved","declined","needs_information","duplicate"].includes(input.status))update.status=input.status;if(["low","normal","high","urgent"].includes(input.priority))update.priority=input.priority;if(input.assigned_to===null||/^[0-9a-f-]{36}$/i.test(input.assigned_to||""))update.assigned_to=input.assigned_to;if(input.duplicate_of===null||/^[0-9a-f-]{36}$/i.test(input.duplicate_of||""))update.duplicate_of=input.duplicate_of;if(typeof input.resolution_summary==="string")update.resolution_summary=input.resolution_summary.trim().slice(0,1000)||null;if(typeof input.github_issue_url==="string")update.github_issue_url=/^https:\/\/github\.com\//i.test(input.github_issue_url)?input.github_issue_url.slice(0,500):null;if(update.status==="resolved"||update.status==="declined")update.resolved_at=new Date().toISOString();const before=await supabase(`/rest/v1/product_feedback?id=eq.${adminFeedback[1]}&select=*`);if(!before.length)throw Object.assign(new Error("Feedback not found"),{status:404});await supabase(`/rest/v1/product_feedback?id=eq.${adminFeedback[1]}`,{method:"PATCH",data:update,prefer:"return=minimal"});if(before[0].contact_allowed&&update.status&&update.status!==before[0].status)await notify(before[0].submitted_by,"feedback",`Feedback ${String(update.status).replaceAll("_"," ")}`,before[0].title,`/?feedback=${adminFeedback[1]}`);await auditRequest(req,user.id,"feedback.update","product_feedback",adminFeedback[1],{before:{status:before[0].status,priority:before[0].priority,assigned_to:before[0].assigned_to},after:update});return json(res,200,{status:"updated"},origin)}
