...
async function requireSensitiveMfa(req){const policy=await configValue("security",{step_up_for_sensitive_actions:false});if(policy.step_up_for_sensitive_actions&&jwtPayload((req.headers.authorization||"").replace(/^Bearer /,"")).aal!=="aal2")throw Object.assign(new Error("Recent multi-factor authentication is required"),{status:403,code:"mfa_required"})}
