...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if(req.method==="GET"&&adminUserDetail){await requireRole(req,["admin"]);const id=adminUserDetail[1];const [profiles,authUser,sightings,comments,sessions,events,assignments,roleDefinitions]=await Promise.all([supabase(`/rest/v1/profiles?id=eq.${id}&select=*`),authAdmin(`/users/${id}`),supabase(`/rest/v1/sightings?reporter_id=eq.${id}&select=id`),supabase(`/rest/v1/sighting_comments?commenter_id=eq.${id}&select=id`),supabase(`/rest/v1/app_sessions?user_id=eq.${id}&select=id,device,last_seen_at,created_at,revoked_at&order=last_seen_at.desc&limit=20`),supabase(`/rest/v1/security_events?user_id=eq.${id}&select=id,event_type,outcome,details,created_at&order=created_at.desc&limit=30`),supabase(`/rest/v1/user_roles?user_id=eq.${id}&select=role_key,reason,expires_at,created_at`),supabase("/rest/v1/rbac_roles?active=eq.true&select=key,display_name,description&order=display_name.asc")]);if(!profiles.length)throw Object.assign(new Error("User not found"),{status:404});const grants=assignments.length?await supabase(`/rest/v1/rbac_role_permissions?role_key=in.(${assignments.map(v=>v.role_key).join(",")})&select=role_key,permission_key`):[],factors=(authUser.factors||[]).map(({id,factor_type,status,created_at,updated_at,friendly_name})=>({id,factor_type,status,created_at,updated_at,friendly_name}));return json(res,200,{profile:profiles[0],auth:{id:authUser.id,email:authUser.email,created_at:authUser.created_at,last_sign_in_at:authUser.last_sign_in_at,email_confirmed_at:authUser.email_confirmed_at,factors},counts:{reports:sightings.length,comments:comments.length},sessions,security_events:events,access:{assignments,available_roles:roleDefinitions,effective_permissions:[...new Set(grants.map(v=>v.permission_key))],explanations:grants.map(v=>({permission:v.permission_key,role:v.role_key}))}},origin)}
