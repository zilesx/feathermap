...
async function moderationContext(row,full=false){
  ...
  const reporterIds=[...new Set(reports.map(item=>item.reporter_id).filter(Boolean))],reporterProfiles=reporterIds.length?await supabase(`/rest/v1/profiles?id=in.(${reporterIds.join(",")})&select=id,display_name,first_name,last_name`):[],reporterNames=new Map(reporterProfiles.map(item=>[item.id,item.display_name||[item.first_name,item.last_name].filter(Boolean).join(" ")||"Flyway member"]));
  ...
  return{...row,content,sighting,owner,reports:reports.map(item=>({...item,reported_by:reporterNames.get(item.reporter_id)||"Flyway member"})),comments,media:media.map(item=>({...item,url:`/api/media/${item.id}`})),history,duplicates};
}
