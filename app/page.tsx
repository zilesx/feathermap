...
export default function Home() {
    ...
    const activeActivityCount = useAggregateActivity ? activityCanvasPoints.length + bandedReportCount : visibleReports.length;
    ...
    useEffect(() => { const requestId = ++heatRequest.current; if (!preferencesReady || zoom >= 7 || taxonFilters !== null) {
        ...
        return;
    } setHeatCells([]); setHeatLoading(true); const timer = window.setTimeout(() => request(`/api/map/heatmap?${rangeQuery()}&zoom=${zoom.toFixed(1)}`).then(data => { if (requestId === heatRequest.current)
        setHeatCells(data.cells || []); }).catch(() => { if (requestId === heatRequest.current)
        setHeatCells([]); }).finally(() => { if (requestId === heatRequest.current)
        setHeatLoading(false); }), 220); return () => window.clearTimeout(timer); }, [preferencesReady, Math.floor(zoom * 2), timeDays, customRange.start, customRange.end, taxonFilters]);
    ...
    async function openReport() { if (!token) {
        setAuthOpen(true);
        return;
    } setReportType("seen"); setReportBirds([freshReportBird(species, amount)]); setObservedAt(localDateTime()); setReportLocation({ source: "current", label: "Current device location", latitude: null, longitude: null, accuracy_meters: 0 }); setPlaceResults([]); setPopularLocations([]); setReporting(true); try {
    ...
    async function submit() { if (!observedAt) {
        ...
        setReportLocation({ source: "map", label: "Choose a point on the map", latitude: null, longitude: null, accuracy_meters: 0 });
        setStatus("Location is unavailable. Choose the observation point on the map.");
        setReporting(false);
