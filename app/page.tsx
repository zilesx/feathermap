...
type OwnedMapReport = LiveReport & { latitude: number; longitude: number; owner: true };
...
export default function Home() {
    ...
    const [activityView, setActivityView] = useState<"community" | "mine">("community");
    ...
    const [ownedMapError, setOwnedMapError] = useState("");
    ...
    const visibleReports = useMemo(() => { const ownerIds=new Set(ownedMapReports.map(report=>report.id)),owned=ownedMapReports.map(report=>({ ...report, zone_latitude:Number(report.latitude), zone_longitude:Number(report.longitude), x:position(Number(report.latitude),1), y:position(Number(report.longitude),2), size:76, color:categoryColor(categoryFor(report.species)), age:age(report.occurred_at), owner:true } as MapReport));return [...owned,...reports.filter(report=>!ownerIds.has(report.id))].filter(r => preferences.visible_groups.includes(categoryFor(r.species)) && (filter === "all" || filter === categoryFor(r.species)) && (taxonFilters === null || taxonFilters.includes(r.subspecies || r.species))); }, [reports, ownedMapReports, filter, taxonFilters, preferences.visible_groups, catalog, categories]);
    const ownedDisplayReports = useMemo<MapReport[]>(() => ownedMapReports.map(report => ({ ...report, zone_latitude: Number(report.latitude), zone_longitude: Number(report.longitude), x: position(Number(report.latitude), 1), y: position(Number(report.longitude), 2), size: 76, color: categoryColor(categoryFor(report.species)), age: age(report.occurred_at), owner: true })), [ownedMapReports, catalog, categories]);
    ...
    const selectableReports = useMemo(() => [...ownedDisplayReports, ...reports.filter(report => !ownedMapReports.some(owned => owned.id === report.id))], [ownedDisplayReports, reports, ownedMapReports]);
    ...
    useEffect(() => { if (!preferencesReady || !token) { setOwnedMapReports([]); setOwnedMapError(""); return; } setOwnedMapError(""); request(`/api/sightings/mine/map?${rangeQuery()}`, { headers: { Authorization: `Bearer ${token}` } }).then(data => setOwnedMapReports(data.reports || [])).catch(error => { setOwnedMapReports([]); setOwnedMapError(error instanceof Error ? error.message : "Your reports could not be loaded. Try again."); }); }, [preferencesReady, token, timeDays, customRange.start, customRange.end]);
    ...
    const reportList = (items: MapReport[], empty: string) => <div className="activity-list" aria-live="polite">{items.length ? items.map(r => <button className={`activity-row ${r.owner ? "owned-report-row" : ""}`} aria-label={`Open ${displayLabel(r.species)} ${r.owner ? "your report" : `report by ${r.reporter_name || "FeatherMap member"}`}`} key={r.id} onClick={() => showReport(r.id)}><span className={`duck-badge ${r.color}`} aria-hidden="true">⌁</span><span><b>{displayLabel(r.species)}{r.owner && <em className="owner-label">Your report</em>}</b><small>{r.flock_size} birds · {labels[r.behavior] || r.behavior?.replaceAll("_", " ")} · {r.owner ? (r.status === "expired" ? "Historical" : "Active") : r.reporter_name || "FeatherMap member"}</small></span><span className="row-meta">{r.age}<small>{r.has_photo ? "Photo · " : ""}{r.report_type === "banded" ? "Banded" : `${r.confidence}% confidence`}</small></span></button>) : <div className="panel-empty">{empty}</div>}</div>;
    ...
    {panel !== "map" && <div className="panel-backdrop" onClick={() => setPanel("map")}><section className="content-panel" onClick={e => e.stopPropagation()}><div className="panel-head"><div><span className="eyebrow">FEATHERMAP NETWORK</span><h2>{panel === "activity" ? activityView === "mine" ? "My reports" : "Community activity" : panel === "saved" ? "Saved sightings" : panel === "admin" ? "Administration" : "Your account"}</h2></div><div className="panel-actions">{panel === "activity" && <button onClick={() => { load(); if (token) { setOwnedMapError(""); request(`/api/sightings/mine/map?${rangeQuery()}`, { headers: { Authorization: `Bearer ${token}` } }).then(data => setOwnedMapReports(data.reports || [])).catch(error => setOwnedMapError(error instanceof Error ? error.message : "Your reports could not be loaded.")); } }}>↻ Refresh</button>}<button className="panel-close" onClick={() => setPanel("map")} aria-label="Close and return to map">×</button></div></div>{panel === "activity" && <><div className="activity-tabs" role="tablist" aria-label="Activity views"><button role="tab" aria-selected={activityView === "community"} className={activityView === "community" ? "active" : ""} onClick={() => setActivityView("community")}>Community Activity</button><button role="tab" aria-selected={activityView === "mine"} className={activityView === "mine" ? "active" : ""} onClick={() => setActivityView("mine")} disabled={!token}>My Reports{token && <span>{ownedDisplayReports.length}</span>}</button></div>{activityView === "community" ? reportList(reports, "No recent reports yet.") : <>{ownedMapError && <div className="panel-error" role="alert">{ownedMapError}</div>}{reportList(ownedDisplayReports, token ? "You have no reports in this timeframe." : "Sign in to view your reports.")}</>}</>}{panel === "saved" && reportList(savedReports, "Save useful sightings from the map and they’ll appear here on this device.")}{panel === "admin" && renderAdmin()}{panel === "more" && <div className="account-card"><div className="account-avatar">{profile?.display_name?.slice(0, 2).toUpperCase() || "IN"}</div><h3>{profile?.display_name || "Hunter"}</h3><p>Manage your profile, preferences, security, and activity using the controls below.</p>{profile && <nav className="account-actions" aria-label="Account actions"><button onClick={openProfile}>Edit profile</button><button onClick={openNotifications}>Notifications</button><button onClick={openSecurity}>Password &amp; security</button><button onClick={openHarvestJournal}>Harvest journal</button><button onClick={() => { setActivityView("mine"); setPanel("activity"); }}>My reports</button>{profile.role !== "user" && <a href="/admin">Admin Portal</a>}<button className="signout" onClick={logout}>Sign out</button></nav>}</div>}</section></div>}
