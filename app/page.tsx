...
export default function Home() {
    ...
    useEffect(() => { if (!preferencesReady || !token) { setOwnedMapReports([]); return; } request(`/api/sightings/mine/map?${rangeQuery()}`, { headers: { Authorization: `Bearer ${token}` } }).then(data => setOwnedMapReports(data.reports || [])).catch(() => setOwnedMapReports([])); }, [preferencesReady, token, timeDays, customRange.start, customRange.end]);
  ...
  {harvestOpen && <div className="modal-wrap harvest-layer" onClick={() => setHarvestOpen(false)}><section className="modal harvest-modal" onClick={event => event.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">PRIVATE HUNTING JOURNAL</span><h2>Log harvest</h2></div><button onClick={() => setHarvestOpen(false)} aria-label="Close harvest journal">×</button></div><p className="privacy-copy">Harvest details are private by default. Public statistics use privacy-safe aggregates only.</p><div className="harvest-grid"><label>Outing started<input type="datetime-local" value={harvestDraft.occurred_at} onChange={event => setHarvestDraft({ ...harvestDraft, occurred_at: event.target.value })}/></label><label>Outing ended<input type="datetime-local" value={harvestDraft.ended_at} onChange={event => setHarvestDraft({ ...harvestDraft, ended_at: event.target.value })}/></label><label>Hunters<NumericStepper label="hunter count" value={Number(harvestDraft.hunter_count)} min={1} max={100} onChange={value => setHarvestDraft({ ...harvestDraft, hunter_count: value ?? 1 })}/></label><label>Birds observed<NumericStepper label="birds observed" value={Number(harvestDraft.observed_count)} min={0} max={1000000} onChange={value => setHarvestDraft({ ...harvestDraft, observed_count: value ?? 0 })}/></label></div><h3>Recovered harvest</h3><div className="harvest-items">{harvestItems.map((item, index) => <div key={index}><select value={item.species_slug} onChange={event => setHarvestItems(items => items.map((current, i) => i === index ? { ...current, species_slug: event.target.value, subspecies_slug: "" } : current))}>{catalog.map(speciesItem => <option key={speciesItem.slug} value={speciesItem.slug}>{speciesItem.display_name}</option>)}</select><select value={item.subspecies_slug} onChange={event => setHarvestItems(items => items.map((current, i) => i === index ? { ...current, subspecies_slug: event.target.value } : current))}><option value="">No subspecies</option>{subspeciesCatalog.filter(sub => sub.species_slug === item.species_slug).map(sub => <option key={sub.slug} value={sub.slug}>{sub.display_name}</option>)}</select><NumericStepper label={`recovered ${displayLabel(item.species_slug)} count`} value={Number(item.recovered_count||0)} min={0} max={1000000} onChange={value=>setHarvestItems(items=>items.map((current,i)=>i===index?{...current,recovered_count:value??0}:current))}/><button aria-label="Remove harvest line" disabled={harvestItems.length === 1} onClick={() => setHarvestItems(items => items.filter((_, i) => i !== index))}>×</button></div>)}</div><button className="secondary-action" onClick={() => setHarvestItems(items => [...items, { species_slug: catalog[0]?.slug || "mallard", subspecies_slug: "", recovered_count: 0 }])}>Add species</button><label>Notes<textarea maxLength={2000} value={harvestDraft.notes} onChange={event => setHarvestDraft({ ...harvestDraft, notes: event.target.value })}/></label><button className="share" onClick={saveHarvest}>Save private harvest</button>{harvestStatus && <p className="form-status">{harvestStatus}</p>}</section></div>}
