...
export default function Home() {
    ...
    <div ref={mapRef} className="map-canvas" onPointerDown={mapPointerDown} onPointerMove={mapPointerMove} onPointerUp={mapPointerEnd} onPointerCancel={mapPointerEnd}>
      ...
      {visibleReports.filter(report => report.report_type === "banded").map(report => { const point = markerPosition(report); return <button key={`banded-star-${report.id}`} className="banded-map-star" data-report-id={report.id} style={{ left: point.left, top: point.top, "--category-color": categoryColor(categoryFor(report.species)) } as CSSProperties} onClick={event => { event.stopPropagation(); selectReport(report.id); }} onContextMenu={event => event.stopPropagation()} aria-label={`Banded ${displayLabel(report.species)} report. Show activity card.`}/>; })}
      {zoom <= 5 && <div className="heat-layer view-density" role="img" aria-label="Privacy-safe aggregate bird activity density">{heatCells.map((cell, i) => { const point = markerPosition({ zone_latitude: cell.cell_latitude, zone_longitude: cell.cell_longitude } as MapReport); return <span key={i} className="heat-cell density-dot" style={{ left: point.left, top: point.top, "--heat-intensity": Math.max(.2, cell.intensity), "--bird-volume": Math.max(.12, cell.intensity), "--report-count": Math.min(1, cell.report_count / 20) } as CSSProperties} title={`About ${cell.estimated_birds || cell.report_count} birds across ${cell.report_count} protected reports`}/>; })}</div>}
      ...
      {zoom > 7 && visibleReports.filter(r => r.report_type !== "banded").map(r => { const point = markerPosition(r); return <button aria-label={`Show ${displayLabel(r.species)} activity card, ${r.flock_size} birds`} key={r.id} data-report-id={r.id} onClick={e => { if (e.detail === 0)
      ...
      {zoom >= 7 && zoom < 12 && <div className="protected-report-dots" aria-label="Protected bird activity reports">{visibleReports.filter(r => r.report_type !== "banded").map(r => { const point = markerPosition(r); return <button key={`dot-${r.id}`} data-report-id={r.id} className={`report-density-dot ${selected?.id === r.id ? "selected" : ""}`} style={{ left: point.left, top: point.top, "--bird-volume": flockDotScale(r.flock_size) } as CSSProperties} onClick={e => { if (e.detail === 0)
            selectReport(r.id); }} onContextMenu={e => e.stopPropagation()} aria-label={`Show ${displayLabel(r.species)} activity card, ${r.flock_size} birds`} title={`${displayLabel(r.species)} · ${r.flock_size} birds`}/>; })}</div>}
      {zoom >= 12 && visibleReports.filter(r => r.report_type !== "banded").map(r => { const point = markerPosition(r); return <button aria-label={`Show ${displayLabel(r.species)} activity card, ${r.flock_size} birds`} key={`local-${r.id}`} data-report-id={r.id} onClick={e => { if (e.detail === 0)
    ...
    {panel !== "map" && <div className="panel-backdrop" onClick={() => setPanel("map")}><section className="content-panel" onClick={e => e.stopPropagation()}><div className="panel-head"><div><span className="eyebrow">FEATHERMAP NETWORK</span><h2>{panel === "activity" ? "Recent activity" : panel === "saved" ? "Saved sightings" : panel === "admin" ? "Administration" : "Your account"}</h2></div><div className="panel-actions">{panel === "activity" && <button onClick={load}>↻ Refresh</button>}<button className="panel-close" onClick={() => setPanel("map")} aria-label="Close and return to map">×</button></div></div>{panel === "activity" && reportList(reports, "No recent reports yet.")}{panel === "saved" && reportList(savedReports, "Save useful sightings from the map and they’ll appear here on this device.")}{panel === "admin" && renderAdmin()}{panel === "more" && <div className="account-card"><div className="account-avatar">{profile?.display_name?.slice(0, 2).toUpperCase() || "IN"}</div><h3>{profile?.display_name || "Hunter"}</h3><p>Manage your profile, preferences, security, and activity using the controls below.</p>{profile && <nav className="account-actions" aria-label="Account actions"><button onClick={openProfile}>Edit profile</button><button onClick={openNotifications}>Notifications</button><button onClick={openSecurity}>Password &amp; security</button><button onClick={openHarvestJournal}>Harvest journal</button><button onClick={() => setBandedOpen(true)}>Report banded bird</button>{profile.role !== "user" && <a href="/admin">Admin Portal</a>}<button className="signout" onClick={logout}>Sign out</button></nav>}</div>}</section></div>}
  ...
  {panel === "more" && profile && !profileOpen && !securityOpen && !notificationsOpen && !harvestOpen && !bandedOpen && <div className="account-quick-actions"><button onClick={openProfile}>Edit profile</button><button onClick={openNotifications}>Notifications</button><button onClick={openSecurity}>Password &amp; security</button><button onClick={openHarvestJournal}>Harvest journal</button><button onClick={() => setBandedOpen(true)}>Report banded bird</button>{profile.role !== "user" && <a href="/admin">Admin Portal</a>}<button onClick={logout}>Sign out</button></div>}
